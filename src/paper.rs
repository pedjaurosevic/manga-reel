//! Real Paper 2 — port of ~/screen/shaders/fx/paper/real-paper-2.frag
//! Applied in page/pixel space so the sheet moves with the comic (no parallax).

#![allow(clippy::too_many_arguments)]

use rayon::prelude::*;

/// Apply Real Paper 2 in-place on RGBA8 page pixels (premultiplied not required; straight RGBA).
pub fn apply_real_paper_2(rgba: &mut [u8], width: u32, height: u32) {
    ...
    if rgba.len() < w * h * 4 {    let src = rgba.to_vec();
    ...
    out.par_chunks_mut(w * 4)
        ...
        .for_each(|(y, row)| {
            for x in 0..w {
                ...
                let wicked = (
                    mix(r0, 0.5 * (r_a + r_b), wick_amt),
                    mix(g0, 0.5 * (g_a + g_b), wick_amt),
                    mix(b0, 0.5 * (b_a + b_b), wick_amt),
                );

                let coord = (x as f32, y as f32);
                let hgt = paper_height(coord);
                let h_light =
                    paper_height((coord.0 - light_dir.0 * 1.5, coord.1 - light_dir.1 * 1.5));
                let relief = (hgt - h_light) * 1.2;

                let print_mask = smoothstep(0.22, 0.82, lum);
                let field_mask = 1.0 - print_mask;
                let diffuse = 1.0 + relief * mix(0.08, 0.145, field_mask);

                let glow = smoothstep(0.62, 0.98, lum);
                let mut matte = (
                    wicked.0 * mix(1.0, 0.90, glow),
                    wicked.1 * mix(1.0, 0.90, glow),
                    wicked.2 * mix(1.0, 0.90, glow),
                );
                matte = (
                    mix(matte.0, cream_ink.0, glow * 0.22),
                    mix(matte.1, cream_ink.1, glow * 0.22),
                    mix(matte.2, cream_ink.2, glow * 0.22),
                );
                ...
                let chroma_scale = mix(1.0, 0.86, neon);
                matte = (
                ...
                let printed = (
                    mix(matte.0 * (1.0 - press), dark_paper.0, holdout),
                    mix(matte.1 * (1.0 - press), dark_paper.1, holdout),
                    mix(matte.2 * (1.0 - press), dark_paper.2, holdout),
                );

                let mottle =
                    (vnoise((coord.0 * 0.48, coord.1 * 0.48)) - 0.5) * 0.055 * print_mask;
                let printed = (
                ...
                let base = (
                    mix(printed.0, printed.0 + stock.0, floor_mix),
                    mix(printed.1, printed.1 + stock.1, floor_mix),
                    mix(printed.2, printed.2 + stock.2, floor_mix),
                );
                ...
                row[i] = (textured.0 * vignette).clamp(0.0, 1.0).mul_add(255.0, 0.5) as u8;
                row[i + 1] = (textured.1 * vignette).clamp(0.0, 1.0).mul_add(255.0, 0.5) as u8;
                row[i + 2] = (textured.2 * vignette).clamp(0.0, 1.0).mul_add(255.0, 0.5) as u8;
                row[i + 3] = a0;
            }
        });
...
fn hash21(p: (f32, f32)) -> f32 {
    // Dave Hoskins: p3 = fract(p.xyx * 0.1031); p3 += dot(p3, p3.yzx + 33.33);
    let mut px = fract(p.0 * 0.1031);
    let mut py = fract(p.1 * 0.1031);
    let mut pz = fract(p.0 * 0.1031); // xyx → z = x
    // dot(p3, p3.yzx + 33.33) = px*(py+33.33) + py*(pz+33.33) + pz*(px+33.33)
...
fn paper_height(p: (f32, f32)) -> f32 {
    ...
    let fibers = (f1 + f2) * 0.5;
    tooth * 0.65 + fibers * 0.35
}
...
/// Save current Hyprland screen_shader and clear it (comics get in-app paper).
